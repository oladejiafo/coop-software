 ...
 if (isset($_POST['submit']))
 {
   switch ($_POST['submit'])
   {
     case 'Update':
      if (Trim($catid) != "")
      {
        $query_update = "UPDATE `asset category` SET `ID`='$catid',`Category` = '$category' WHERE `Company` ='" .$_SESSION['company']. "' AND  `ID` = '$catid'";
        $result_update = mysqli_query($conn,$query_update) or die(mysqli_error());
            ...
            $query_update_Log = "Insert into `Monitor` (`User Category`, `User Name`,`Date Logged on`, `Time Logged on`,`File Used`,`Details`,`Company`) 
                  VALUES ('" . $rows['access_name'] . "','" . ucfirst($_SESSION['name']) . "', '" . date('Y/m/d') . "', '" . date('h:i A') . "','Asset Category Update','Modified Asset Category: " . $category . "', '" .$_SESSION['company']. "')";
      ...
      { 
        ...
        $result_insert = mysqli_query($conn,$query_insert) or die(mysqli_error());
            ...
            $query_insert_Log = "Insert into `Monitor` (`User Category`, `User Name`,`Date Logged on`, `Time Logged on`,`File Used`,`Details`,`Company`) 
                  VALUES ('" . $rows['access_name'] . "','" . ucfirst($_SESSION['name']) . "', '" . date('Y/m/d') . "', '" . date('h:i A') . "','Asset Category Update','Added Asset Category: " . $category . "', '" .$_SESSION['company']. "')";
     ...
     case 'Delete':
      {
       $query_delete = "DELETE FROM `asset category` WHERE `Company` ='" .$_SESSION['company']. "' AND  `ID` = '$catid';";
       $result_delete = mysqli_query($conn,$query_delete) or die(mysqli_error());          
            ...
            $query_delete_Log = "Insert into `Monitor` (`User Category`, `User Name`,`Date Logged on`, `Time Logged on`,`File Used`,`Details`,`Company`) 
                  VALUES ('" . $rows['access_name'] . "','" . ucfirst($_SESSION['name']) . "', '" . date('Y/m/d') . "', '" . date('h:i A') . "','Asset Category Update','Deleted Asset Category: " . $category . "', '" .$_SESSION['company']. "')";
