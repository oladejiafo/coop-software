...
<tr align='center'>
 <td>
     <h3><center><u>STATEMENT OF FINANCIAL CASH FLOWS REPORT</u></center></h3>
...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#ccCCcc" id="table2">
 <?php
   $limit      = 50; 
   @$page=$_GET['page'];
   $query_count    = "SELECT * FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` in ('Cash, Cheques and Other Cash Items','Accounts with Banks and other Financial Institutions','Loans and Advances','Prepayments and Other Receivables','Property and Equipment','Investments')";
...
############################
       $sqlt="SELECT sum(`Amount`) as amount1 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Interest Income on Loans to Clients','Interest Income on Loans to Non-Clients','Interest Income from Balances','Interest Income from Accounts with Banks and other','Interest Income from Approved Investments','Other Interest Income') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt = mysqli_query($conn,$sqlt) or die('Could not look up user data; ' . mysqli_error());
       $rowt = mysqli_fetch_array($resultt);
       $amtt1=$rowt['amount1'];
 
       $sqlt2="SELECT sum(`Amount`) as amount2 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Interest Income Expense on Clients Deposits','Interest Income Expense on Clients Voluntary Savings','Interest Income Expense on Deposits of Non-Clients','Interest Income Expense on Deposits of Non-Clients','Interest Income Expense on Short-term Loans','Interest Income Expense on Long-term Loans','Interest Income Expense on other Borrowings','Fees Paid for Loans form Banks and other Organization','Commissions Paid for Loans from Banks and other Organizations') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt2 = mysqli_query($conn,$sqlt2) or die('Could not look up user data; ' . mysqli_error());
       $rowt2 = mysqli_fetch_array($resultt2);
       $amtt2=$rowt2['amount2'];

       $sqlt3="SELECT sum(`Amount`) as amount3 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Client Fees Paid','Client Penalties','Fee from Payment services and Intra-country Transfers','Fee from Offering Insurance Products as Agent','Other Operating Income') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt3 = mysqli_query($conn,$sqlt3) or die('Could not look up user data; ' . mysqli_error());
       $rowt3 = mysqli_fetch_array($resultt3);
       $amtt3=$rowt3['amount3'];

       $sqlt4="SELECT sum(`Amount`) as amount4 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Gain on Foreign Exchange','Gain on Disposal of Property and Equipment','Gain on Sale of Investments','Rental Income','Reversal of Provisions for Liabilities and Charges','Write-back of Principal Received on Loans Previously Written Off','Write-back of Interest Received on Loans Previously Written Off','Other Income Items') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt4 = mysqli_query($conn,$sqlt4) or die('Could not look up user data; ' . mysqli_error());
       $rowt4 = mysqli_fetch_array($resultt4);
       $amtt4=$rowt4['amount4'];

       $sqlt5="SELECT sum(`Amount`) as amount5 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Loss on Foreign Exchange',
...
'Other Indirect and Miscellaneous Taxes') and `Date` between '$filter' and '$filter2' group by `Classification`";
       ...
       $sqlt6="SELECT sum(`Amount`) as amount6 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Income Tax') and `Date` between '$filter' and '$filter2' group by `Classification`";
...
########ADJUSTMENTS
       $sqlt="SELECT sum(`Amount`) as dep FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Depreciation and Amortisation') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt = mysqli_query($conn,$sqlt) or die('Could not look up user data; ' . mysqli_error());
       $rowt = mysqli_fetch_array($resultt);
       $amtDep=$rowt['dep'];
 
       $sqlt2="SELECT sum(`Amount`) as armo FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Accumulated Amortisation and Depreciation') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt2 = mysqli_query($conn,$sqlt2) or die('Could not look up user data; ' . mysqli_error());
       $rowt2 = mysqli_fetch_array($resultt2);
       $amtAmor=$rowt2['armo'];

       $sqlt3="SELECT sum(`Amount`) as gdisp FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Gain on Disposal of Property and Equipment') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt3 = mysqli_query($conn,$sqlt3) or die('Could not look up user data; ' . mysqli_error());
       $rowt3 = mysqli_fetch_array($resultt3);
       $amtGDisp=$rowt3['gdisp'];

       $sqlt3A2="SELECT sum(`Amount`) as ldisp FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Loss on Sale of Property and Equipment') and `Date` between '$filter' and '$filter2' group by `Classification`";
       ...
       $sqlt4="SELECT sum(`Amount`) as rev FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` like 'Revaluation%' and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt4 = mysqli_query($conn,$sqlt4) or die('Could not look up user data; ' . mysqli_error());
       $rowt4 = mysqli_fetch_array($resultt4);
       $amtRev=$rowt4['rev'];
...
########Changes in Working Capital
       $sqlL="SELECT sum(`Amount`) as rec FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Short-term Receivables') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL = mysqli_query($conn,$sqlL) or die('Could not look up user data; ' . mysqli_error());
       $rowL = mysqli_fetch_array($resultL);
       $amtRec=$rowL['rec'];
 
       $sqlL2="SELECT sum(`Amount`) as prep FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Prepayments') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL2 = mysqli_query($conn,$sqlL2) or die('Could not look up user data; ' . mysqli_error());
       $rowL2 = mysqli_fetch_array($resultL2);
       $amtPrep=$rowL2['prep'];

       $sqlL3="SELECT sum(`Amount`) as oass FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Other Fixed Assets') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL3 = mysqli_query($conn,$sqlL3) or die('Could not look up user data; ' . mysqli_error());
       $rowL3 = mysqli_fetch_array($resultL3);
       $amtOAss=$rowL3['oass'];

       $sqlL31="SELECT sum(`Amount`) as trad FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Accounts Payable') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL31 = mysqli_query($conn,$sqlL31) or die('Could not look up user data; ' . mysqli_error());
       $rowL31 = mysqli_fetch_array($resultL31);
       $amtTrad=$rowL31['trad'];

       $sqlL4="SELECT sum(`Amount`) as olib FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` like '%Other Liabilities%' and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL4 = mysqli_query($conn,$sqlL4) or die('Could not look up user data; ' . mysqli_error());
       $rowL4 = mysqli_fetch_array($resultL4);
       $amtOLib=$rowL4['olib'];

       $sqlL5="SELECT sum(`Amount`) as otax FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Other Indirect and Miscellaneous Taxes') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL5 = mysqli_query($conn,$sqlL5) or die('Could not look up user data; ' . mysqli_error());
       $rowL5 = mysqli_fetch_array($resultL5);
       $amtOTax=$rowL5['otax'];

       $sqlL6="SELECT sum(`Amount`) as itax FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Income Tax') and `Date` between '$filter' and '$filter2' group by `Classification`";
...
<td>
<?php
echo "<a target='blank' href='rptscf.php?filter=$filter&filter2=$filter2'> Print this Report</a> &nbsp;";
echo "| <a target='blank' href='expscf.php?filter=$filter&filter2=$filter2'> Export this Report</a> &nbsp; ";
?>
</td>
</tr>
</Table
