...
&nbsp;
 ...
 <?php
   $limit      = 50; 
   @$page=$_GET['page'];
   $query_count    = "SELECT * FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` in ('Interest Income','Interest Expense','Fee and commission Income','Finance Income','Operating Expenses','Tax on Profit')";
...
<?php
   $result = mysqli_query ($conn,"SELECT distinct `Category`,`Description`,`Remarks` FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` like 'Interest Income%' or `Category` like '%Interest%Expense%' or `Category` like 'Fee and Charges%' or `Category` like 'Other Income Accounts' or `Category` like 'Other Charges' or `Category` like 'General and Administrative Expenses' or `Category` like 'Loan Servicing%' or `Category` like 'Supervision and Licensing Fees%' or `Category` like 'Promotional Expenses%' or `Category` like 'Tax and Licenses%' group by `Description` order by `Category` LIMIT $limitvalue, $limit"); 
