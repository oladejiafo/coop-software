...
&nbsp;
  <input type="text" name="filter" id="inputFieldA" style="height:35px;width:120px">
&nbsp
  <input type="text" name="filter2" id="inputFieldB" style="height:35px;width:120px">
&nbsp
 ...
 @$filter2=$_REQUEST["filter2"];
  ...
  {  
   $query_count    = "SELECT * FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Date` = '" . date('Y-m-d') . "' and `Type`='Income'";
  }
  else if ($cmbFilter=="All")
  {  
   $query_count    = "SELECT * FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Type`='Income'";
  }
  else if ($cmbFilter=="Date Range")
  {  
   $query_count    = "SELECT * FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  (`Date` between '" . $filter . "' and '" . $filter2 . "') and `Type`='Income'"; 
  }
  else if ($cmbFilter=="Account Number")
  {  
   $query_count    = "SELECT * FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  (`Account Number` = '" . $filter . "' or `Account Number` = '" . $filter2 . "') and `Type`='Income'"; 
  }
...
<?php
  if ($cmbFilter=="" or $cmbFilter=="Today" or empty($cmbFilter))
  {  
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Note`, `Date`,`Amount`,`Account Number` FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Date` = '" . date('Y-m-d') . "' and `Type`='Income' order by `Date` desc LIMIT $limitvalue, $limit"); 
  }
  else if ($cmbFilter=="All")
  {
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Note`, `Date`,`Amount`,`Account Number` FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Type`='Income' order by `Date` desc LIMIT $limitvalue, $limit");   
  }
  else if ($cmbFilter=="Date Range")
  {  
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Note`, `Date`,`Amount`,`Account Number` FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  (`Date` between '" . $filter . "' and '" . $filter2 . "') and `Type`='Income' order by `Date` desc LIMIT $limitvalue, $limit"); 
  }
  else if ($cmbFilter=="Account Number")
  {  
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Note`, `Date`,`Amount`,`Account Number` FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  (`Account Number` = '" . $filter . "' or `Account Number` = '" . $filter2 . "') and `Type`='Income' order by `Date` desc LIMIT $limitvalue, $limit"); 
  }
   ...
   {	
      ...
      $sqlw="SELECT * FROM `membership` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Membership Number`='$acctno'";
