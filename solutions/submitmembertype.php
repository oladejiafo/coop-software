 ...
 if (isset($_POST['submit']))
 {
   switch ($_POST['submit'])
   {
     case 'Modify':
      if (Trim($type) != "")
      {
        $query_update = "UPDATE `membership type` SET `Type`='$type' WHERE `Company` ='" .$_SESSION['company']. "' AND  `ID` = '$ID'";
     ...
     case 'Add':
      if (Trim($type) != "")
      { 
        $query_insert = "Insert into `membership type` (`Type`,`Company`) 
     ...
     case 'Delete':
      {
       $query_delete = "DELETE FROM `membership type` WHERE `Company` ='" .$_SESSION['company']. "' AND  `ID` = '$ID'";
