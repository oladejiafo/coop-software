...
<?php 
   ...
   $result = mysqli_query ($conn,"SELECT `Account Number`,`Transactor`, `Date`, `Deposit` FROM `transactions` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Transaction Type`='Deposit' and `Date` = '" . date('Y-m-d') . "' and `Officer` ='" . $_SESSION['name'] . "' order by `ID`"); 
   $result2 = mysqli_query ($conn,"SELECT `Account Number`,`Transactor`, `Date`, `Withdrawal` FROM `transactions` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Transaction Type`='Withdrawal' and `Date` = '" . date('Y-m-d') . "' and `Officer` ='" . $_SESSION['name'] . "' order by `ID`"); 
...
{
$sqlH1="SELECT * FROM `membership` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Membership Number`='$eclass'";
