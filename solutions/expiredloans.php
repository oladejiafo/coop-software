...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#ccCCcc" id="table2">
 <?php
   ...
   $resultX = mysqli_query ($conn,"SELECT distinct `Loan Date`,`ID`, `Loan Duration` FROM `loan` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Due Date` < '" . date('Y-m-d') . "' and `Balance` > 0 and `Loan Status`='Active' group by `Account Number`"); 
/*
...
/*
   $due=date('Y-m-d', strtotime('+' . $durX . ' month',strtotime($dateX)));
        ...
        $query_update = "UPDATE `loan` SET `Due Date`='$due' WHERE `Company` ='" .$_SESSION['company']. "' AND  `ID` = '$idX'";
...
*/
   ...
   $result = mysqli_query ($conn,"SELECT distinct `Account Number`,`ID`, `Loan Amount`, `Loan Date`, `Loan Duration`, `Payment todate`, `Balance`,`Loan Type` FROM `loan` WHERE `Company` ='" .$_SESSION['company']. "' AND  ($valt) and `Balance` > 0 and `Loan Status`='Active' group by `ID`"); 
