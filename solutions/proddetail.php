...
<td align="right">
 ...
 @$page=$_GET['page'];
   ...
   $query_count = "SELECT * FROM `customer` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Account Type` in ('Current','Savings') group by `Account Type`";
 ...
 if ($cmbFilter=="" or $cmbFilter=="Today" or empty($cmbFilter))
  {  
   $queryQ  = "SELECT `customer`.`Account Number`,`customer`.`Account Type` FROM `customer` inner join `transactions` on `customer`.`Account Number`=`transactions`.`Account Number` WHERE `Company` ='" .$_SESSION['company']. "' AND  `customer`.`Account Type`='$accttype' and `Date` = '" . date('Y-m-d') . "' and (`Deposit` >0 OR `Withdrawal`>0) order by `customer`.`Account Number`";
  }
  else if ($cmbFilter=="All")
  {  
   $queryQ  = "SELECT `customer`.`Account Number`,`customer`.`Account Type` FROM `customer` inner join `transactions` on `customer`.`Account Number`=`transactions`.`Account Number` WHERE `Company` ='" .$_SESSION['company']. "' AND  `customer`.`Account Type`='$accttype' and (`Deposit` >0 OR `Withdrawal`>0) order by `customer`.`Account Number`";
  }
  else if ($cmbFilter=="Date Range")
  {  
   $queryQ  = "SELECT `customer`.`Account Number`,`customer`.`Account Type` FROM `customer` inner join `transactions` on `customer`.`Account Number`=`transactions`.`Account Number` WHERE `Company` ='" .$_SESSION['company']. "' AND  `customer`.`Account Type`='$accttype' and (`Date` between '" . $filter . "' and '" . $filter2 . "') and (`Deposit` >0 OR `Withdrawal`>0) order by `customer`.`Account Number`";
  }
...
$swit=0;
   ...
   {
    $queryQq = "SELECT `Account Number`,`Branch`,`Officer`,`Surname`,`First Name` FROM `customer` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Account Number`='$acctno' and `Account Type`='$accttype'";
...
#    $totrw  = mysqli_num_rows($resultQq);
 ...
 if ($cmbFilter=="" or $cmbFilter=="Today" or empty($cmbFilter))
  {  
   $queryct = "SELECT sum(`transactions`.`Deposit`) as sumdep, sum(`transactions`.`Withdrawal`) as sumwit FROM `transactions` inner join `customer` on `transactions`.`Account Number`=`customer`.`Account Number` WHERE `Company` ='" .$_SESSION['company']. "' AND  `customer`.`Account Number`='$acctno' and `Date` = '" . date('Y-m-d') . "' and (`Deposit` >0 OR `Withdrawal`>0)"; 
  }
  else if ($cmbFilter=="All")
  {  
   $queryct = "SELECT sum(`transactions`.`Deposit`) as sumdep, sum(`transactions`.`Withdrawal`) as sumwit FROM `transactions`  inner join `customer` on `transactions`.`Account Number`=`customer`.`Account Number` WHERE `Company` ='" .$_SESSION['company']. "' AND  `customer`.`Account Number`='$acctno' and (`Deposit` >0 OR `Withdrawal`>0)";
  }
  else if ($cmbFilter=="Date Range")
  {  
   $queryct = "SELECT sum(`transactions`.`Deposit`) as sumdep, sum(`transactions`.`Withdrawal`) as sumwit FROM `transactions` inner join `customer` on `transactions`.`Account Number`=`customer`.`Account Number` WHERE `Company` ='" .$_SESSION['company']. "' AND  `customer`.`Account Number`='$acctno' and (`Date` between '" . $filter . "' and '" . $filter2 . "') and (`Deposit` >0 OR `Withdrawal`>0)"; 
  }
