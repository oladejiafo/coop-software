...
$sql="SELECT * FROM `treasury` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Direction`='Out' and `ID`='$idd'";
$result = mysql_query($sql,$conn) or die('Could not look up user data; ' . mysql_error());
$row = mysql_fetch_array($result);

$sql2="SELECT * FROM `treasury` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Direction`='In' and `ID`='$ide'";
    ...
    <tr>
      <td>
         <select name="curnote" size="1">
           ...
           $sql = "SELECT * FROM `currency` WHERE `Company` ='" .$_SESSION['company']. "'";
      ...
      <td>
        <select name="cashier" size="1">
           ...
           $sql = "SELECT * FROM `login` WHERE `Company` ='" .$_SESSION['company']. "' AND  `access_lvl` in (1,2)";
           $result_cur = mysql_query($sql,$conn) or die('Could not list value; ' . mysql_error());
           while ($rows = mysql_fetch_array($result_cur)) 
           {
...
if(empty($idd))
{
 ...
 if (($_SESSION['access_lvl'] == 5) or ($_SESSION['access_lvl'] == 4))
 {
   $querry = "SELECT `ID`,`Note`,`Amount` FROM `treasury` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Given By`='" . ucfirst($_SESSION['name']) . "' and `Date`='" . date('Y-m-d') . "' and `Direction`='Out' order by `Note` desc";
   $resultrp=mysql_query($querry);
 } else {
   $querry = "SELECT `ID`,`Note`,`Amount` FROM `treasury` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Given By`='Olazz' and `Date`='" . date('Y-m-d') . "' and `Direction`='Out' order by `Note` desc";
   $resultrp=mysql_query($querry);
 }
   $tamtr=0;
...
if(empty($ide))
{
 ...
 if (($_SESSION['access_lvl'] == 5) or ($_SESSION['access_lvl'] == 1))
 {
   $querry2 = "SELECT `ID`,`Note`,`Amount` FROM `treasury` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Given By`='" . ucfirst($_SESSION['name']) . "' and `Date`='" . date('Y-m-d') . "' and `Direction`='In' order by `Note` desc";
   $resultrp2=mysql_query($querry2);
 } else {
   $querry2 = "SELECT `ID`,`Note`,`Amount` FROM `treasury` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Given By`='olaxx' and `Date`='" . date('Y-m-d') . "' and `Direction`='In' order by `Note` desc";
   $resultrp2=mysql_query($querry2);
 }
