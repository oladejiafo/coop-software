...
<?php
 ...
 $filter=$_REQUEST["filter"];
  ...
  {  
   $result = mysqli_query ($conn,"SELECT `User Category`, `User Name` , `Date Logged On`, `Time Logged On`,`File Used`,`Details` From `Monitor` WHERE `Company` ='" .$_SESSION['company']. "' AND  `User Category`='" . $filter . "' order by `Date Logged On` Desc, `Time Logged On` desc");    
  }
  else if (trim($cmbFilter)=="User Name")
  {  
   $result = mysqli_query ($conn,"SELECT `User Category`, `User Name` , `Date Logged On`, `Time Logged On`,`File Used`,`Details` From `Monitor` WHERE `Company` ='" .$_SESSION['company']. "' AND  `User Name`='" . $filter . "' order by `Date Logged On` Desc, `Time Logged On` desc");
  }
  else if (trim($cmbFilter)=="Date Logged")
  {  
   $result = mysqli_query ($conn,"SELECT `User Category`, `User Name` , `Date Logged On`, `Time Logged On`,`File Used`,`Details` From `Monitor` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Date Logged On`='" . $filter . "' order by `Date Logged On` Desc, `Time Logged On` desc");
  }
