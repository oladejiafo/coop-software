...
#   while(list($dateX,$idX,$durX)=mysqli_fetch_row($resultX)) 
   ...
   $result = mysqli_query ($conn,"SELECT distinct `Account Number`,`ID`, `Loan Amount`, `Loan Date`, `Loan Duration`, `Payment todate`, `Balance`,`Loan Type`,`PBalance` FROM `loan` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Balance` > 0 and `Loan Status`='Active' group by `ID`"); 
