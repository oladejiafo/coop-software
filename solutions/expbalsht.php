...
$phn=$rw['Phone'];
 ...
 $filename = "Balsheet_" . date('Ymd') . $filter . ".xls";
...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#ccCCcc" id="table2">
 <?php
   ...
   $result = mysqli_query ($conn,"SELECT distinct `Category` FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` in ('Fixed Assets','Current Assets','Current Liabilities')"); 
...
$ttt=0;
    ...
    {	
      ...
      $result2 = mysqli_query ($conn,"SELECT `Description`,`Category` FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category`= '$cat'"); 
