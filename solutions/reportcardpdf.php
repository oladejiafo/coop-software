...
if (!isset($_SESSION['usaid']) & ($_SESSION['ac_lvl'] != 5) & ($_SESSION['ac_lvl'] != 6))
{
  if ($_SESSION['ac_lvl'] != 1)
  {
 ...
 $reslt = mysqli_query($conc, $sqr) or die('Could not look up user data; ' . mysqli_error());
 ...
 $yr=$rw['Year'];
 $term=$rw['Term'];
 $tbegin=$rw['Term Begin'];
 
 @$cmbTerm=$_REQUEST['cmbTerm'];
 @$cmbClass=$_REQUEST['cmbClass'];
 @$cmbYear=$_REQUEST['cmbYear'];
 @$SNO=$_REQUEST['SNO'];
 
 $sql="SELECT * FROM `exams` WHERE `Student Number`='" .$SNO."' and `Year` =" . $cmbYear . " and `Term` like '" . $cmbTerm . "%'  and `Class` like '" . $cmbClass . "%' and `Company`='" . $_SESSION['company'] . "'";
 $result = mysqli_query($conc, $sql) or die('Could not look up user data; ' . mysqli_error());
 @$row = mysqli_fetch_array($result);
 
 $sql2="SELECT * FROM `student` WHERE `Student Number`='$SNO' and `Company`='" . $_SESSION['company'] . "'";
 $result2 = mysqli_query($conc, $sql2) or die('Could not look up user data; ' . mysqli_error());
 @$row2 = mysqli_fetch_array($result2);
 $cls=$row2['Class'];
 $clsa=$row2['Class Alias'];
 
 $sql3="SELECT count(`Student Number`) as ctot FROM `student` WHERE `Class`='$cmbClass' and `Admission Mode` !='Inactive' and `Company`='" . $_SESSION['company'] . "'";
 $result3 = mysqli_query($conc, $sql3) or die('Could not look up user data; ' . mysqli_error());
 @$row3 = mysqli_fetch_array($result3);
 $cltot=$row3['ctot'];
 
 $sql4="SELECT count(`Student No`) as atp FROM `attendance` WHERE `Attendance`='Present' and `Class`='$cmbClass' and `Student No`='$SNO' and `Term` like '" . $cmbTerm . "%' and `Year` = " . $cmbYear . " and `Company`='" . $_SESSION['company'] . "'";
 $result4 = mysqli_query($conc, $sql4) or die('Could not look up user data; ' . mysqli_error());
 @$row4 = mysqli_fetch_array($result4);
 $atpr1=$row4['atp'];
 
 $sql42="SELECT count(`Student No`) as atp FROM `attendance` WHERE `Attendance2`='Present' and `Class`='$cmbClass' and `Student No`='$SNO' and `Term` like '" . $cmbTerm . "%' and `Year` = " . $cmbYear . " and `Company`='" . $_SESSION['company'] . "'";
 $result42 = mysqli_query($conc, $sql42) or die('Could not look up user data; ' . mysqli_error());
 @$row42 = mysqli_fetch_array($result42);
 $atpr2=$row42['atp'];
 $atpr=$atpr1;
 
 $sql5="SELECT count(`Student No`) as ata FROM `attendance` WHERE `Attendance`='Absent' and `Class`='$cmbClass' and `Student No`='$SNO' and `Term` like '" . $cmbTerm . "%' and `Year` = " . $cmbYear . " and `Company`='" . $_SESSION['company'] . "'";
 $result5 = mysqli_query($conc, $sql5) or die('Could not look up user data; ' . mysqli_error());
 @$row5 = mysqli_fetch_array($result5);
 $atab1=$row5['ata'];
 
 $sql52="SELECT count(`Student No`) as ata FROM `attendance` WHERE `Attendance2`='Absent' and `Class`='$cmbClass' and `Student No`='$SNO' and `Term` like '" . $cmbTerm . "%' and `Year` = " . $cmbYear . " and `Company`='" . $_SESSION['company'] . "'";
 $result52 = mysqli_query($conc, $sql52) or die('Could not look up user data; ' . mysqli_error());
...
{
   $pix= "images/pics/$cpfolder/logo.jpg";
} else {
   $pix= "images/logo.jpg";
}
...
{
    //Logo
    $this->Image($pix,10,10,35);
    //Arial bold 15
    $this->SetFont('Arial','B',20);
    //Move to the right
    $this->Cell(80);
    //Title
    $this->Cell(30,15,'CHILD\'S TERMINAL REPORT',0,0,'C');
    //Line break
    $this->Ln(18);
...
/*
   $queryi="SELECT `Surname`,`First Name`,`email`,`Contact Number`,`Bank Account Name`,`Bank Name`,`Bank Account Number` FROM `customer` WHERE `Account Number` = '" . trim($acctno) . "' order by `ID`";
   $resulti=mysqli_query($conn,$queryi);
   while(list($snami,$fnami,$emaili,$foni,$oacctname,$obankname,$oacctnum)=mysqli_fetch_row($resulti))
   {
   $sql = "SELECT `ID`,`Loan Date`,`Loan Amount`,`Loan Requested`,`Loan Duration`,`Monthly Interest`,`Interest Rate`,`Monthly Principal` FROM `loan` WHERE `Account Number`='" . $acctno . "' order by `ID` desc limit 0,1";
...
$pdf->SetFont('Times','',12);
   $queryxp = "SELECT `ID`,`Loan Date`,`Loan Amount`,`Loan Duration`,`Monthly Interest`,`Monthly Principal` FROM `loan` WHERE `Account Number`='" . $acctno . "' order by `ID` desc limit 0,1";
