...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#ccCCcc" id="table2">
 <?php
   $limit      = 50; 
   @$page=$_GET['page'];
   $query_count    = "SELECT * FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` not in ('Fixed Assets','Current Assets','Current Liabilities')";
...
$yr2=$yr1-1;
 ...
 #  echo "<TR bgcolor='#C0C0C0'><TH width='40%'><b> Description</TH><TH width='30%'><b> N</TH><TH width='30%'><b> N</TH></TR>";
   ...
   $result = mysqli_query ($conn,"SELECT distinct `Category`,`Remarks` FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` not in ('Fixed Assets','Current Assets','Current Liabilities') LIMIT $limitvalue, $limit"); 
...
$ttt2=0;
    ...
    {	
      ...
      $result2 = mysqli_query ($conn,"SELECT `Description`,`Category`,`Remarks` FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category`= '$cat' LIMIT $limitvalue, $limit"); 
      while(list($descr,$cat1,$typ)=mysqli_fetch_row($result2)) 
      {	
       if ($typ=="Expense")
       { $sg='-1'; } else { $sg='1';}
       $sqlt="SELECT sum(`Amount`) as amount FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification`='$descr' and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt = mysqli_query($conn,$sqlt) or die('Could not look up user data; ' . mysqli_error());
       $rowt = mysqli_fetch_array($resultt);

       $sqlt2="SELECT sum(`Amount`) as amount FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification`='$descr' and `Date` between '$filter' and '$filter2' and year(`Date`) = '$yr2' group by `Classification`";
...
#   $res = mysqli_query ($conn,"SELECT sum(`Total Cost`) as tsoc, sum(`Bank Amount`) as amtb,sum(`Imprest Amount`) as amti FROM `sales` WHERE `Company` ='" .$_SESSION['company']. "' AND  month(`sales`.`Sales Date`)=month('" . Date('Y-m-d') . "')"); 
