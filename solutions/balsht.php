...
<form action="report.php" method="GET">
 <tr><td>
  ...
  <input type="text" name="filter" style="width:200px;height:35px" value="2015-01-01" id="filter">{YYYY-MM-DD}
  <input type="hidden" name="cmbReport" value="Balance Sheet">
 </td>
 <td>
  Enter Date Range (Ending): <br>
  <input type="text" name="filter2" style="width:200px;height:35px" value="<?php echo date('Y-m-d'); ?>" id="filter2">{YYYY-MM-DD}
 </td>
<td> 
  <input type="submit" value="Generate" name="submit" style="width:170px;height:35px">
</td></tr>
<br>
</form>
</table>
...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#ccCCcc" id="table2">
 <?php
   $limit      = 50; 
   @$page=$_GET['page'];
   $query_count    = "SELECT * FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` in ('Fixed Assets','Current Assets','Current Liabilities')";
...
#   echo "<TR bgcolor='#C0C0C0'><TH width='40%'><b> Description</TH><TH width='15%'><b> N</TH><TH width='15%'><b> N</TH><TH width='15%'><b> N</TH><TH width='15%'><b> N</TH></TR>";
   ...
   $result = mysqli_query ($conn,"SELECT distinct `Category` FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` in ('Fixed Assets','Current Assets','Current Liabilities') LIMIT $limitvalue, $limit"); 
...
$ttt2y2=0;
    ...
    {	
      ...
      $result2 = mysqli_query ($conn,"SELECT `Description`,`Category` FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category`= '$cat' LIMIT $limitvalue, $limit"); 
      while(list($descr,$cat1)=mysqli_fetch_row($result2)) 
      {	
       $sqlt="SELECT sum(`Amount`) as amount FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification`='$descr' and `Type`='Income' and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt = mysqli_query($conn,$sqlt) or die('Could not look up user data; ' . mysqli_error());
       $rowt = mysqli_fetch_array($resultt);

       $sqlt2="SELECT sum(`Amount`) as amount FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification`='$descr' and `Type`='Expenditure' and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt2 = mysqli_query($conn,$sqlt2) or die('Could not look up user data; ' . mysqli_error());
       $rowt2 = mysqli_fetch_array($resultt2);     

       $sqlty2="SELECT sum(`Amount`) as amount FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification`='$descr' and `Type`='Income' and year(`Date`) = '$yr2' group by `Classification`";
       $resultty2 = mysqli_query($conn,$sqlty2) or die('Could not look up user data; ' . mysqli_error());
       $rowty2 = mysqli_fetch_array($resultty2);

       $sqlt2y2="SELECT sum(`Amount`) as amount FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification`='$descr' and `Type`='Expenditure' and year(`Date`) = '$yr2' group by `Classification`";
