...
<form  action="report.php" method="POST">
 ...
 <tr><td>
  ...
  ?> 
   ...
   <input type="hidden" name="cmbReport" style="width:120px;height:30px" value="Expenditure Report">
 </td>
 <td>
...
<table border="0" width="99%" cellspacing="1" bgcolor="#FFFFFF" id="table1">
<tr align='center'>
 <td><b>
...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#ccCCcc" id="table2">
 ...
 @$filter2=$_REQUEST["filter2"];
  ...
  {  
   $query_count    = "SELECT * FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Date` = '" . date('Y-m-d') . "' and `Type`='Expenditure'";
  }
  else if ($cmbFilter=="All")
  {  
   $query_count    = "SELECT * FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Type`='Expenditure'";
  }
  else if ($cmbFilter=="Date Range")
  {  
   $query_count    = "SELECT * FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  (`Date` between '" . $filter . "' and '" . $filter2 . "') and `Type`='Expenditure'"; 
  }
  ...
  if ($cmbFilter=="" or $cmbFilter=="Today" or empty($cmbFilter))
  {  
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Source`, `Date`,`Amount` FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Date` = '" . date('Y-m-d') . "' and `Type`='Expenditure' order by `Date` desc LIMIT $limitvalue, $limit"); 
  }
  else if ($cmbFilter=="All")
  {
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Source`, `Date`,`Amount` FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Type`='Expenditure' order by `Date` desc LIMIT $limitvalue, $limit");   
  }
  else if ($cmbFilter=="Date Range")
  {  
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Source`, `Date`,`Amount` FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  (`Date` between '" . $filter . "' and '" . $filter2 . "') and `Type`='Expenditure' order by `Date` desc LIMIT $limitvalue, $limit"); 
  }
...
<td>
<?php
echo "<a target='blank' href='rptexpense.php?cmbFilter=$cmbFilter&filter=$filter&filter2=$filter2&acctno=$acctno'> Print this Report</a> &nbsp;";
echo "<a target='blank' href='expexpense.php?cmbFilter=$cmbFilter&filter=$filter&filter2=$filter2&acctno=$acctno'> Export this Report</a> &nbsp;";
