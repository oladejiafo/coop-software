...
if(empty($idd))
{
 $idd="999";
  ...
  echo "<TR><TH align='center'><b><u>Notes/Coins </b></u>&nbsp;</TH><TH align='right' colspan=2><b><u>Amount</b></u>&nbsp;</TH></TR>";
   ...
   $querry = "SELECT `ID`,`Note`,`Amount` FROM `treasury` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Date`='" . date('Y-m-d') . "' and `Direction`='Out' and (`Given By`='" . ucfirst($_SESSION['name']) . "' or `Given To`='" . ucfirst($_SESSION['name']) . "')  order by `Note` desc";
...
if(empty($ide))
{
 $ide="999";
  ...
  echo "<TR><TH align='center'><b><u>Notes/Coins </b></u>&nbsp;</TH><TH align='right' colspan=2><b><u>Amount</b></u>&nbsp;</TH></TR>";
   ...
   $querry2 = "SELECT `ID`,`Note`,`Amount` FROM `treasury` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Date`='" . date('Y-m-d') . "' and `Direction`='In' and (`Given By`='" . ucfirst($_SESSION['name']) . "' or `Given To`='" . ucfirst($_SESSION['name']) . "') order by `Note` desc";
