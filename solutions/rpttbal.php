...
$yr2=$yr1-1;
   ...
   $queryDR="SELECT distinct `Category`,`Code`,`Description` FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` in (
...
'Cashier Shortage and Overage', 'Tax on Profit or Loss', 'Other Charges', 'Loans and Advances', 'Property and Equipment', 'Prepayments and Other Receivables', 'Investments', 'Accounts with Banks and other Financial Institutions', 'Cash, Cheques and Other Cash Items'
    ...
    { 
      ############## DEBIT ##############
       ...
       $sqlr="SELECT sum(`Amount`) as dr FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` = '$descrr' group by `Classification`";
   ...
   $queryCR="SELECT `Code`,`Category`,`Description` FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` in ('Interest Income on Loans and Advances', 'Interest Income other than on Loans and Advances'
...
,'Interest Payable', 'Accounts Payable', 'Taxes Payable','Deferred Revenue', 'Suspense and/or Clearing Accounts','Other Liability Accounts', 'Capital', 'Retained Earnings/ (Deficit)', 'Net Income (Loss)', 'Dividends Declared') group by `Category` order by `Category` desc";
    ...
    { 
      ...
      { 
       $sqlt="SELECT sum(`Amount`) as cr FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` = '$descr' group by `Classification`";
