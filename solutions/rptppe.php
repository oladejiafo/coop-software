...
<tr><td colspan=1 width='460'><h2><left>PROPERTY, PLANT and EQUIPMENT REPORT</left></h2></td></tr>
</table>
<div align="left">
...
<?php
   $limit      = 50; 
   @$page=$_GET['page'];
   $query_count    = "SELECT * FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` in ('PROPERTY AND EQUIPMENT')";
...
$yr2=$yr1-1;
   ...
   $result = mysqli_query ($conn,"SELECT distinct `Category`,`Remarks` FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Category` in ('PROPERTY AND EQUIPMENT') LIMIT $limitvalue, $limit"); 
...
$ttt5=0;
    ...
    {	
      ...
      echo "<TR align='center' bgcolor='#dcdfdf' colspan=1><font face='Verdana' color='#ccffff' style='font-size: 12pt'><TH width='40%'>" . $rmm . strtoupper($cat) . "</TH></font><TH colspan=6 width='30%'></TH></TR>";
       ...
       $sqlt="SELECT sum(`Amount`) as amount1 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Heavy Equipment', 'Plant and Machinery') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt = mysqli_query($conn,$sqlt) or die('Could not look up user data; ' . mysqli_error());
       $rowt = mysqli_fetch_array($resultt);
       $amtt=$rowt['amount1'];
 
       $sqlt2="SELECT sum(`Amount`) as amount2 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Furniture and Fixture','Furniture and Fittings') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt2 = mysqli_query($conn,$sqlt2) or die('Could not look up user data; ' . mysqli_error());
       $rowt2 = mysqli_fetch_array($resultt2);
       $amtt2=$rowt2['amount2'];

       $sqlt3="SELECT sum(`Amount`) as amount3 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Computer and other Equipment','Computer Equipment') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt3 = mysqli_query($conn,$sqlt3) or die('Could not look up user data; ' . mysqli_error());
       $rowt3 = mysqli_fetch_array($resultt3);
       $amtt3=$rowt3['amount3'];

       $sqlt4="SELECT sum(`Amount`) as amount4 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Vehicles','Motor Vehicle') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt4 = mysqli_query($conn,$sqlt4) or die('Could not look up user data; ' . mysqli_error());
       $rowt4 = mysqli_fetch_array($resultt4);
       $amtt4=$rowt4['amount4'];

       $sqlt5="SELECT sum(`Amount`) as amount5 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Office Equipment') and `Date` between '$filter' and '$filter2' group by `Classification`";
