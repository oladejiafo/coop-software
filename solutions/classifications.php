...
#           echo '<option selected>' . $cmbFilter . '</option>';
           $sql = "SELECT * FROM `heads` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Description` not like '%Cash%'";
...
<h2><font face='Verdana' color='#000000' style='font-size: 13pt'><b>Classifications Reports</b></left></font></h2>
 ...
 if ($filter=="" or $filter2=="" or empty($filter) or empty($filter2))
 {
  ...
  {  
   ...
   $query_count = "SELECT * FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` = '" . $cmbFilter . "' and `Date` between '$filter' and '$filter2'";
  }
...
<?php
  if ($cmbFilter=="" or $cmbFilter=="- Select Classification to Filter -" or empty($cmbFilter))
  {  
   $query = "SELECT `ID`,`Date`,`Particulars`,`Classification`,`Amount`,`Type` FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' Order By `ID` desc";
  }
  else
  {  
   if ($filter=="" or $filter2=="" or empty($filter) or empty($filter2))
   ...
   $query = "SELECT `ID`,`Date`,`Particulars`,`Classification`,`Amount`,`Type` FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` = '" . $cmbFilter . "' and `Date` between '$filter' and '$filter2'";
  }
