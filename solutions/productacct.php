...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#000000" id="table2">
 ...
 @$filter2=$_REQUEST["filter2"];
   ...
   $query_count = "SELECT * FROM `membership` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Status`='Active' group by `Membership Type`";
   ...
   $queryQ = "SELECT `membership`.`Membership Number`,`membership`.`Membership Type` FROM `membership` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Status`='Active' group by `membership`.`Membership Type`";
...
$swit=0;
 ...
 if ($cmbFilter=="" or $cmbFilter=="Today" or empty($cmbFilter))
  {  
   $queryct = "SELECT sum(`transactions`.`Deposit`) as sumdep, sum(`transactions`.`Withdrawal`) as sumwit FROM `transactions` WHERE `Company` ='" .$_SESSION['company']. "' AND  `transactions`.`Account Number`='$acctno' and `Date` = '" . date('Y-m-d') . "'"; 
  }
  else if ($cmbFilter=="All")
  {  
   $queryct = "SELECT sum(`transactions`.`Deposit`) as sumdep, sum(`transactions`.`Withdrawal`) as sumwit FROM `transactions` WHERE `Company` ='" .$_SESSION['company']. "' AND  `transactions`.`Account Number`='$acctno'";
  }
  else if ($cmbFilter=="Date Range")
  {  
   $queryct = "SELECT sum(`transactions`.`Deposit`) as sumdep, sum(`transactions`.`Withdrawal`) as sumwit FROM `transactions` WHERE `Company` ='" .$_SESSION['company']. "' AND  `transactions`.`Account Number`='$acctno' and (`Date` between '" . $filter . "' and '" . $filter2 . "')"; 
  }
