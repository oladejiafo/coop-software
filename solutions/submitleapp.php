   ...
   switch ($_POST['submit'])
   {
     case 'Proceed to Stage 2 >>':
      if ($_SESSION['stage']==1)
      {
        ...
        $query_insert = "Insert into `lease application` (`First Name`, `Surname`, `Middle Name`, `Maiden Name`, `Marital Status`,`Gender`, `DoB`, `Mobile Number`,`Contact Number`,`Application Date`,`Status`,`Company`) 
        VALUES ('$fname', '$sname', '$mname', '$oname', '$mstatus','$gender', '$dob', '$mobileno','$contactno','$dat','Pending', '" .$_SESSION['company']. "')";
        $result_insert = mysqli_query($conn,$query_insert);

        $sql="SELECT `ID` FROM `lease application` WHERE `Company` ='" .$_SESSION['company']. "' AND  `First Name`='$fname' and `Surname`='$sname' and `Mobile Number`='$mobileno' order by `ID` desc";
...
if ($results[$email]) 
{
        ...
        $query_update = "UPDATE `lease application` SET `Identification Type`='$idtype',`email`='$email',`Children`='$children',`Household`='$household' WHERE `Company` ='" .$_SESSION['company']. "' AND  `ID` = '$id'";
...
#####
     ...
     case 'Proceed to Stage 7 >>':
      if ($_SESSION['stage']==6)
      {
        ...
        $query_update = "UPDATE `lease application` SET `Official Email`='$officialemail',`Industry Type`='$industrytype',`Previous Duration`='$prevdur',`Past Employers`='$pastemployers',`Education Level`='$edulevel',`Institution`='$institution' WHERE `Company` ='" .$_SESSION['company']. "' AND  `ID` = '$id'";
        $result_update = mysqli_query($conn,$query_update);
...
#####
     ...
     case 'Proceed to Stage 8 >>':
      if ($_SESSION['stage']==7)
      {
        ...
        $query_update = "UPDATE `lease application` SET `NK Name`='$nkname',`NK Relationship`='$nkrelationship',`NK Phone`='$nkphone',`NK Address`='$nkaddress', `NK LGA`='$nklga', `NK State`='$nkstate', `NK email`='$nkemail',`NK Employer`='$nkemployer', `NK Job Title`='$nkjobtitle' WHERE `Company` ='" .$_SESSION['company']. "' AND  `ID` = '$id'";
        $result_update = mysqli_query($conn,$query_update);
...
#####
     ...
     case 'SUBMIT':
      if ($_SESSION['stage']==9)
      {
        ...
        $dat=date('Y-m-d');
        $query_update = "UPDATE `lease application` SET `Authorize`='$authorize',`Terms`='$terms',`Application Date`='$dat' WHERE `Company` ='" .$_SESSION['company']. "' AND  `ID` = '$id'";
        $result_update = mysqli_query($conn,$query_update);
