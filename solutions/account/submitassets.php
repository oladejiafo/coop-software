 ...
 if (isset($_POST['submit']))
 {
   switch ($_POST['submit'])
   {
     case 'Update':
      ...
      {
        $query_update = "UPDATE `assets` SET `Location` = '$location',`Code`='$code',`Category`='$category',`Type`='$type',`Name`='$name',
     ...
     case 'Save':
      if (Trim($name) != "")
      { 
        $query_insert = "Insert into `assets` (`Location`,`Code`,`Category`,`Name`,`Description`,`Make`,`Status`,`Quantity`,`Date Acquired`,`Purchase Price`,`Serial No`,`Current Value`,`Depreciation`,`Next Maintenance`,`Comment`,`Sold`,`Date Sold`,`Amount Sold`,`Type`) 
               VALUES ('$location','$code','$category','$name','$description','$make','$status','$quantity','$dateacquired','$pprice','$serialno','$cvalue','$depreciation','$schedule','$comment','$sold','$datesold','$amountsold','$type')";
