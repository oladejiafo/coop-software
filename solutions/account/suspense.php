...
<form  action="suspense.php" method="post">
    ...
    <select size="1" name="cmbFilter" style="height:40px;width:170px; font-size: 12px">
...
<?php
  if (!$cmbFilter=="")
  {  
   $query_count    = "SELECT * FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `" . $cmbFilter . "` like '" . $filter . "%' and `Classification` in ('Suspense Items (unidentified)')";
   ...
   $query="SELECT distinct `Date`, `ID`,`Recipient`,`Classification`,`Amount`,`Particulars` FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `" . $cmbFilter . "` like '" . $filter . "%' and `Classification` in ('Suspense Items (unidentified)') group by `Recipient`,`Date` order by `Date` desc LIMIT $limitvalue, $limit";
...
echo "</div>";
  ...
  {
   $query_count    = "SELECT * FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Suspense Items (unidentified)')";
   $result_count   = mysqli_query($conn,$query_count);     
   $totalrows  = mysqli_num_rows($result_count);
   ...
   $query="SELECT distinct `Date`,`ID`,`Recipient`,`Classification`,`Amount`,`Particulars` FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Suspense Items (unidentified)') group by `Recipient`,`Date` order by `Date` desc LIMIT $limitvalue, $limit";
   $result=mysqli_query($conn,$query);
