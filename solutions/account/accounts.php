...
$sql="SELECT * FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `ID`='$ID' order by `Date` desc";
...
<div align="leftt" style="margin-left:20px;font-size: 12px" class="agileinfo_mail_grids">
	...
	<select  class="input__field input__field--chisato" placeholder=" " name="type" width="31" value="<?php echo $row['Type']; ?>" onChange="getCity('findcity.php?cat='+this.value)" required>  
          <?php  
...
if (!$ID){
?>
  <input type="submit" value="Save" name="submit" style="width:140px; height:35px"> &nbsp;
<?php } 
 else { ?>
  <input type="submit" value="Update" name="submit" style="width:140px; height:35px"> &nbsp; 
  <input type="submit" value="Delete" name="submit" onclick="return confirm('Are you sure you want to Delete?');" style="width:140px; height:35px"> &nbsp; 
<?php
} ?>
  </span>
  </div>
