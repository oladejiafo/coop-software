<?php
  ...
  if (isset($_REQUEST['action'])) 
  {
    switch ($_REQUEST['action']) 
    {
      case 'Login':
        ...
        {
          $cnt=0;
          $sqlXL = "SELECT user_id,access_lvl,username " .
                 "FROM login " .
 ...
 $tnow=date('m') . " " . date('Y');
 $toda=date('Y-m-d');
$queryp = "SELECT  distinct `Account Number`,`ID` FROM `transactions` WHERE `Remark`='Contribution Charge' and (day(`Date`)=" . $tday . ") order by `ID` desc";
...
{ 
 ...
 { 
        $sqlCT="SELECT * FROM `customer` WHERE `Account Number`='$vacctno' order by `ID` desc";
