...
$cmbReport="Loans Report";
 ...
 $filename = "Income_" . date('Ymd') . $filter . ".xls";
...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#ccCCcc" id="table2">
 ...
 @$filter2=$_REQUEST["filter2"];
  ...
  if ($cmbFilter=="" or $cmbFilter=="Today" or empty($cmbFilter))
  {  
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Note`, `Date`,`Amount`,`Account Number` FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Date` = '" . date('Y-m-d') . "' and `Type`='Income' order by `Date` desc"); 
  }
  else if ($cmbFilter=="All")
  {
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Note`, `Date`,`Amount`,`Account Number` FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Type`='Income' order by `Date` desc");   
  }
  else if ($cmbFilter=="Date Range")
  {  
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Note`, `Date`,`Amount`,`Account Number` FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  (`Date` between '" . $filter . "' and '" . $filter2 . "') and `Type`='Income' order by `Date` desc"); 
  }
  else if ($cmbFilter=="Account Number")
  {  
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Note`, `Date`,`Amount`,`Account Number` FROM `sundry` WHERE `Company` ='" .$_SESSION['company']. "' AND  (`Account Number` = '" . $filter . "' or `Account Number` = '" . $filter2 . "') and `Type`='Income' order by `Date` desc"); 
  }
