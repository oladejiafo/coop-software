...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#005B00">
 ...
 @$limit      = 25;
 @$page=$_GET['page'];
 @$query_count    = "SELECT * FROM `loan application` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Status` not in ('Approved','Disapproved')";     
 ...
 $limitvalue = $page * $limit - ($limit);  
    ...
    $query="SELECT `ID`,`Application Date`,`Surname`,`First Name`,`Current Employer`,`Loan Amount`,`Tenor`,`Income` FROM `loan application` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Status` not in ('Approved','Disapproved') order by `Application Date`,`ID` LIMIT $limitvalue, $limit";
