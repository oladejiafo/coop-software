...
and $_POST['username'] !="" )
{
 $sqlzz = "Update login set `password`='" . md5($passwd) . "' WHERE `Company` ='" .$_SESSION['company']. "' AND  `username`='" . $_POST['username'] . "'";
 mysqli_query($conn,$sqlzz);
#########################################
          $sqlt = "SELECT user_id,access_lvl, username,email " .
                 "FROM login " .
                 "WHERE `Company` ='" .$_SESSION['company']. "' AND  `username`='" . $_POST['username'] . "'";
