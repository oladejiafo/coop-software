...
<?php
########ASSETS
       $sqlt="SELECT sum(`Amount`) as amount1 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Cash on Hand','Petty Cash', 'Vault Cash', 'Cheques in Transit', 'Other Cash Accounts') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt = mysqli_query($conn,$sqlt) or die('Could not look up user data; ' . mysqli_error());
       $rowt = mysqli_fetch_array($resultt);
       $amtA1=$rowt['amount1'];
 
       $sqlt2="SELECT sum(`Amount`) as amount2 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Cash on Hand','Petty Cash','Vault Cash','Cheques in Transit', 'Other Cash Accounts') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt2 = mysqli_query($conn,$sqlt2) or die('Could not look up user data; ' . mysqli_error());
       $rowt2 = mysqli_fetch_array($resultt2);
       $amtA2=$rowt2['amount2'];

       $sqlt3="SELECT sum(`Amount`) as amount3 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Consumer Loans to Clients','Business Loans to Clients','Agricultural Loans to Clients','Real Estate Loans to Clients','Savings and Deposit Secured Loans to Clients','Other Short-term Clients Loans','Current and Outstanding Loans to Non-Clients','Restructured Loans to Clients','Restructured Loans to Non-Clients','Past Due Non-Performing Loans (NPL) Account') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt3 = mysqli_query($conn,$sqlt3) or die('Could not look up user data; ' . mysqli_error());
       $rowt3 = mysqli_fetch_array($resultt3);
       $amtA3=$rowt3['amount3'];

       $sqlt3A2="SELECT sum(`Amount`) as amount31 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Held to maturity financial asset') and `Date` between '$filter' and '$filter2' group by `Classification`";
       ...
       $sqlt4="SELECT sum(`Amount`) as amount4 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Prepayments','Short-term Receivables') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt4 = mysqli_query($conn,$sqlt4) or die('Could not look up user data; ' . mysqli_error());
       $rowt4 = mysqli_fetch_array($resultt4);
       $amtA4=$rowt4['amount4'];

       $sqlt5="SELECT sum(`Amount`) as amount5 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Receivables') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt5 = mysqli_query($conn,$sqlt5) or die('Could not look up user data; ' . mysqli_error());
       $rowt5 = mysqli_fetch_array($resultt5);
       $amtA5=$rowt5['amount5'];

       $sqlt6="SELECT sum(`Amount`) as amount6 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Land and Buildings','Office Equipment','Plant and Machinery','Vehicles','Furniture and Fixture','Computer and other Equipment','Intangible Assets','Other Fixed Assets','Accumulated Amortisation and Depreciation') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt6 = mysqli_query($conn,$sqlt6) or die('Could not look up user data; ' . mysqli_error());
       $rowt6 = mysqli_fetch_array($resultt6);
       $amtA6=$rowt6['amount6'];

       $sqlt7="SELECT sum(`Amount`) as amount7 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Investment in CBN Approved Government Securities','Loans to other MFBs','Other Investments') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt7 = mysqli_query($conn,$sqlt7) or die('Could not look up user data; ' . mysqli_error());
       $rowt7 = mysqli_fetch_array($resultt7);
       $amtA7=$rowt7['amount7'];

       $sqlt8="SELECT sum(`Amount`) as amount8 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Deffered tax asset') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt8 = mysqli_query($conn,$sqlt8) or die('Could not look up user data; ' . mysqli_error());
       $rowt8 = mysqli_fetch_array($resultt8);
       $amtA8=$rowt8['amount8'];

       $sqlt9="SELECT sum(`Amount`) as amount9 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Intangible assets') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultt9 = mysqli_query($conn,$sqlt9) or die('Could not look up user data; ' . mysqli_error());
       $rowt9 = mysqli_fetch_array($resultt9);
       $amtA9=$rowt9['amount9'];

       $sqlt9i="SELECT sum(`Amount`) as amount9i FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Specific Reserve for Performing Loans (0 day)', 'Specific Reserve for Pass and Watch Loans (1-30 days)', 'Specific Reserve for Substandard Loans (31-60 days)','Specific Reserve for Doubtful Loans (61-90 days)','Specific Reserve for Lost Loans (>91 days)') and `Date` between '$filter' and '$filter2' group by `Classification`";
       ...
       $sqlt10="SELECT sum(`Amount`) as amount10 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Deferred Grant Revenue','Deferred Revenue due to Donated Fixed Assets','Other Deferred Revenue Items') and `Date` between '$filter' and '$filter2' group by `Classification`";
...
########LIABILITIES
       $sqlL="SELECT sum(`Amount`) as amount1 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Deposit from banks') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL = mysqli_query($conn,$sqlL) or die('Could not look up user data; ' . mysqli_error());
       $rowL = mysqli_fetch_array($resultL);
       $amtL1=$rowL['amount1'];
 
       $sqlL2="SELECT sum(`Amount`) as amount2 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Customers Deposits','Term Deposits') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL2 = mysqli_query($conn,$sqlL2) or die('Could not look up user data; ' . mysqli_error());
       $rowL2 = mysqli_fetch_array($resultL2);
       $amtL2=$rowL2['amount2'];

       $sqlL3="SELECT sum(`Amount`) as amount3 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Income Tax') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL3 = mysqli_query($conn,$sqlL3) or die('Could not look up user data; ' . mysqli_error());
       $rowL3 = mysqli_fetch_array($resultL3);
       $amtL3=$rowL3['amount3'];

       $sqlL31="SELECT sum(`Amount`) as amount31 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Interest Payable on Short-term Bank Loans','Interest Payable on Long-term Bank Loans','Interest Payable on Other Borrowings') and `Date` between '$filter' and '$filter2' group by `Classification`";
       ...
       $sqlL4="SELECT sum(`Amount`) as amount4 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` like '%Audit%' and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL4 = mysqli_query($conn,$sqlL4) or die('Could not look up user data; ' . mysqli_error());
       $rowL4 = mysqli_fetch_array($resultL4);
       $amtL4=$rowL4['amount4'];

       $sqlL5="SELECT sum(`Amount`) as amount5 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Interest Payable on Savings Deposits', 'Interest Payable on Term Deposits', 'Accounts Payable', 'Withholding Tax','Social security Tax', 'Profit Tax', 'Minimum Tax', 'Other Taxes', 'Suspense Items (unidentified)', 'Clearing Account', 'Other Liabilities') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultL5 = mysqli_query($conn,$sqlL5) or die('Could not look up user data; ' . mysqli_error());
       $rowL5 = mysqli_fetch_array($resultL5);
       $amtL5=$rowL5['amount5'];

########EQUITY
       $sqlE="SELECT sum(`Amount`) as amount1 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Share Capital') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultE = mysqli_query($conn,$sqlE) or die('Could not look up user data; ' . mysqli_error());
       $rowE = mysqli_fetch_array($resultE);
       $amtE1=$rowE['amount1'];
 
       $sqlE2="SELECT sum(`Amount`) as amount2 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('Specific Reserve Expense') and `Date` between '$filter' and '$filter2' group by `Classification`";
       $resultE2 = mysqli_query($conn,$sqlE2) or die('Could not look up user data; ' . mysqli_error());
       $rowE2 = mysqli_fetch_array($resultE2);
       $amtE2=$rowE2['amount2'];

       $sqlE3="SELECT sum(`Amount`) as amount3 FROM `cash` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Classification` in ('General Reserve Expense', 'Provision for Loss on Investments and Loans to other Organizations','Provision for Loss on Assets Acquired in Liquidation', 'General Reserves') and `Date` between '$filter' and '$filter2' group by `Classification`";
