...
$sql="SELECT * FROM `interbank` WHERE `Company` ='" .$_SESSION['company']. "' AND  `r_Date`='" . date('Y-m-d') . "' and `ID`='$idd'";
...
if(empty($idd))
{
 ...
 {
   $querry = "SELECT `ID`,`r_Narration`,`r_Amount`,`r_Officer`,`r_Branch`,`Received By` FROM `interbank` WHERE `Company` ='" .$_SESSION['company']. "' AND  `r_Date`='" . date('Y-m-d') . "' order by `ID` desc";
   $resultrp=mysql_query($querry);
 } else {
   $querry = "SELECT `ID`,`r_Narration`,`r_Amount`,`r_Officer`,`r_Branch`,`Received By` FROM `interbank` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Received By`='" . ucfirst($_SESSION['name']) . "' and `r_Date`='" . date('Y-m-d') . "' order by `ID` desc";
   $resultrp=mysql_query($querry);
 }
   $tamtr=0;
echo '
...
#  echo "<TR><TH align='left'><b><u>Notes/Coins </b></u>&nbsp;</TH><TH align='right' colspan=2><b><u>Amount</b></u>&nbsp;</TH></TR>";
 ...
 if (($_SESSION['access_lvl'] == 5) or ($_SESSION['access_lvl'] == 4))
 {
   $querryp = "SELECT `ID`,`p_Narration`,`p_Amount`,`p_Officer`,`p_Branch`,`Paid By` FROM `interbank` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Received By`='" . ucfirst($_SESSION['name']) . "' and `p_Date`='" . date('Y-m-d') . "' order by `ID` desc";
   $resultpp=mysql_query($querryp);
 } else {
   $querryp = "SELECT `ID`,`p_Narration`,`p_Amount`,`p_Officer`,`p_Branch`,`Paid By` FROM `interbank` WHERE `Company` ='" .$_SESSION['company']. "' AND  `p_Date`='" . date('Y-m-d') . "' order by `ID` desc";
   $resultpp=mysql_query($querryp);
 }
   $tamtp=0;
echo '
