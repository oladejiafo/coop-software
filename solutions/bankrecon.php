...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#ccCCcc" id="table2">
 <?php
   $limit      = 50; 
   @$page=$_GET['page'];
   $query_count    = "SELECT * FROM `cheque` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Date` between '$filter' and '$filter2' group by `Type`, `Status`";
   ...
   $result = mysql_query ("SELECT distinct `Particulars`,`Type`,`Status` FROM `cheque` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Date` between '$filter' and '$filter2' group by `Type`, `Status` LIMIT $limitvalue, $limit"); 
...
$ttb=0;
    ...
    {
       $sql_pc="SELECT sum(`Amount`) as amount_pc FROM `cheque` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Type`='Payment' and `Status`='Cleared' and `Date` between '$filter' and '$filter2'";
       $result_pc = mysql_query($sql_pc,$conn) or die('Could not look up user data; ' . mysql_error());
       $rowpc = mysql_fetch_array($result_pc);
     
       $sql_pu="SELECT sum(`Amount`) as amount_pu FROM `cheque` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Type`='Payment' and `Status`='Uncleared' and `Date` between '$filter' and '$filter2'";
       $result_pu = mysql_query($sql_pu,$conn) or die('Could not look up user data; ' . mysql_error());
       $rowpu = mysql_fetch_array($result_pu);

       $sql_rc="SELECT sum(`Amount`) as amount_rc FROM `cheque` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Type`='Receipt' and `Status`='Cleared' and `Date` between '$filter' and '$filter2'";
       $result_rc = mysql_query($sql_rc,$conn) or die('Could not look up user data; ' . mysql_error());
       $rowrc = mysql_fetch_array($result_rc);

       $sql_ru="SELECT sum(`Amount`) as amount_ru FROM `cheque` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Type`='Receipt' and `Status`='Cleared' and `Date` between '$filter' and '$filter2'";
