...
else if ($cmbFilter=="Colour")
{
 ...
 echo "<p><font style='font-size: 9pt'>";
    ...
    echo "<TR><TH><b><u>Stock Code</b></u>&nbsp;</TH><TH><b><u>Product Name </b></u>&nbsp;</TH><TH><b><u>Category </b></u>&nbsp;</TH>
  ...
  {  
   ...
   $query="SELECT `Stock Code`,`Stock Name`,`Category`,`Units in Stock`,`Location` FROM stock WHERE `" . $cmbFilter . "` like '" . $filter . "%' LIMIT $limitvalue, $limit";
   ...
   { 
    ...
    while(list($code,$name,$attribute,$units,$location)=mysql_fetch_row($result))
    {
     echo "<TR><TH> $code</TH><TH><a href = 'stock.php?code=" . $code . "'> $name </a></TH><TH><a href = 'stock.php?code=" . $code . "'>$attribute </a> &nbsp;</TH><TH> $units &nbsp;</TH><TH>$location &nbsp;</TH></TR>";
    }
  ...
  else
  {
   ...
   $query="SELECT `Stock Code`,`Stock Name`,`Category`,`Units in Stock`,`Location` FROM stock LIMIT $limitvalue, $limit";
   $result=mysql_query($query);
