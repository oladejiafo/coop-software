 ...
 if (isset($_POST['submit']))
 {
   switch ($_POST['submit'])
   {
     case 'Update':
      ...
      {
        $query_update = "UPDATE `query` SET `Staff Number`='$serviceno',`Reference` = '$reference',`Date`='$date', `Action`='$action',
        ...
        $result_update = mysql_query($query_update) or die(mysql_error());
            ...
            $query_update_Log = "Insert into `monitor` (`User Category`, `User Name`,`Date Logged on`, `Time Logged on`,`File Used`,`Details`) 
                  VALUES ('" . $rows['access_name'] . "','" . ucfirst($_SESSION['name']) . "', '" . date('Y/m/d') . "', '" . date('h:i A') . "','Query Update','Modified query record for Staff: " . $serviceno . ", Query Reference: " . $reference . "')";
     ...
     case 'Save':
      if (Trim($reference) != "" & Trim($serviceno) != "")
      { 
        $query_insert = "Insert into `query` (`Staff Number`,`Reference`, `Date`,`Action`,`Issued By`,`Remark`) 
               VALUES ('$serviceno','$reference','$date','$action','$issuedby','$remark')
               ";
        ...
        $result_insert = mysql_query($query_insert) or die(mysql_error());
            ...
            $query_insert_Log = "Insert into `monitor` (`User Category`, `User Name`,`Date Logged on`, `Time Logged on`,`File Used`,`Details`) 
                  VALUES ('" . $rows['access_name'] . "','" . ucfirst($_SESSION['name']) . "', '" . date('Y/m/d') . "', '" . date('h:i A') . "','Query Update','Added query record for Staff: " . $serviceno . ", Query Reference: " . $reference . "')";
     ...
     case 'Delete':
      {
       $query_delete = "DELETE FROM `query` WHERE `Reference` = '$reference';";
       $result_delete = mysql_query($query_delete) or die(mysql_error());          
            ...
            $query_delete_Log = "Insert into `monitor` (`User Category`, `User Name`,`Date Logged on`, `Time Logged on`,`File Used`,`Details`) 
                  VALUES ('" . $rows['access_name'] . "','" . ucfirst($_SESSION['name']) . "', '" . date('Y/m/d') . "', '" . date('h:i A') . "','Query Update','Deleted query record for Staff: " . $serviceno . ", Query Reference: " . $reference . "')";
