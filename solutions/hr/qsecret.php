...
$sql="SELECT `staff`.`Staff Number`,`staff`.Surname,`staff`.Firstname,`staff`.Dept,`staff`.`Present Rank`,`staff`.Level,`staff`.Step, 
       `query`.`Date`,`query`.`Action`, `query`.`Reference`,`query`.`Issued By`,`query`.`Remark` FROM staff left join `query` on `staff`.`Staff Number`=`query`.`Staff Number` WHERE `staff`.`Staff Number`='$SNO'";
...
<div align="left">
<b><i><font size="3" face="Verdana" color="#008000">[QUERIES]</font></i></b>
...
<?php
    ...
    $query="SELECT `Date`,`Action`, `Reference`,`Issued By`,`Remark`,`Staff Number` FROM `query` WHERE `Staff Number`='$SNO'";
    $result=mysql_query($query);

    while(list($date,$action,$reference,$issuedby,$remark,$serviceNo)=mysql_fetch_row($result))
    {
     echo "<TR><TH><a href = 'Query.php?RNO=" .$reference . "'> $date </a> &nbsp;</TH><TH>$action &nbsp;</TH>
...
<TABLE width='30%' border='1' cellpadding='1' cellspacing='0' align='center' bordercolor="#005B00">
  ...
  <?php
     echo "<TR>
               <TH><a href ='Query.php'> Add New Query </a>&nbsp;</TH>
           </TR>"; 
  ?>
</TABLE> 
 </fieldset>
