 ...
 if (isset($_POST['submit']))
 {
   switch ($_POST['submit'])
   {
     case 'Update':
      ...
      {
        $query_update = "UPDATE `qualification` SET `Qualification`='$qualification' WHERE `Qualification` = '$qualification';";
     ...
     case 'Save':
      if (Trim($qualification) != "")
      { 
        $query_insert = "Insert into `qualification` (`Qualification`) 
     ...
     case 'Delete':
      {
       $query_delete = "DELETE FROM `qualification` WHERE `Qualification` = '$qualification';";
