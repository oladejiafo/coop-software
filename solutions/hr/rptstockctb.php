...
$EW_TABLE_DISTINCT_SQL_SELECT = "SELECT DISTINCT `Location` FROM `stock`";
$EW_TABLE_DISTINCT_SQL_WHERE = "";
$EW_TABLE_DISTINCT_SQL_ORDERBY = "`Location`";
...
<table id="ewReport" class="ewTable">
	...
	<tr class="ewTableRow">
		...
		<td align="center" class="ewRptColHeader" colspan="<?php echo @$ncolspan; ?>" nowrap>
			Location
		</td>
	</tr>
	<tr>
...
<?php echo $x_Location ?>
	...
	?>
<!-- Dynamic columns end -->
		<?php $x_Location = "Total"; ?>
		<td class="ewTableHeader" valign="top">
<b><?php echo $x_Location ?>
</b></td>
	</tr>
<?php
if ($nTotalGrps > 0) {
...
while ($row) {
	...
	if (intval($nGrpCount) >= intval($nStartGrp) && $nGrpCount <= $nStopGrp) {
		...
		if ($x_Stock_Name <> "" && $ox_Stock_Name == $x_Stock_Name && !ChkLvlBreak(2)) {
			$gx_Stock_Name = "";
...
?>
	...
	<tr<?php echo $sItemRowClass; ?>>
		...
		<!-- Stock Name -->
		<td class="ewRptGrpField2"><?php $tx_Stock_Name = $x_Stock_Name; $x_Stock_Name = $gx_Stock_Name; ?>
...
function GetColumns() {
	...
	for ($colcnt = 1; $colcnt <= $ncol; $colcnt++) {
		$sFld = ew_CrossTabField("SUM", "`Units in Stock`", "`Location`", "", $col[$colcnt][0], "'", $colcnt);
		$sSqlFlds .= ", " . $sFld;
	}
