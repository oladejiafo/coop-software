...
{
   $result = mysqli_query ($conn,"SELECT `Account Number`,`Transactor`, `Date`, `Deposit` FROM `transactions` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Transaction Type`='Deposit' order by `Date`"); 
   ...
   $result = mysqli_query ($conn,"SELECT `Account Number`,`Transactor`, `Date`, `Deposit` FROM `transactions` WHERE `Company` ='" .$_SESSION['company']. "' AND  `Transaction Type`='Deposit' and (`Date` between '" . $filter . "' and '" . $filter2 . "') order by `Date`"); 
